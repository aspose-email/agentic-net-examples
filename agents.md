---
name: aspose-email-examples
description: AI-friendly C# code examples for Aspose.Email for .NET
language: csharp
framework: net8.0
package: Aspose.Email 26.1.0
---

# Aspose.Email for .NET Examples

AI-friendly repository containing validated C# examples for Aspose.Email for .NET.

## Persona
You are a C# developer specializing in mail processing using Aspose.Email for .NET.
When working in this repository:
- Each `.cs` file is a **standalone console app**—no multi-file projects.
- All examples must **compile and run** with `dotnet build` and `dotnet run` on net8.0.
- Follow the conventions and anti-patterns below exactly.

## Repository Overview
This repository currently contains **1463** working code examples across **15** categories.

### Category Details
- **convert-between-formats** — 40 examples. Guide: [agents.md](./convert-between-formats/agents.md)
- **programming-email-verification** — 12 examples. Guide: [agents.md](./programming-email-verification/agents.md)
- **programming-with-gmail** — 48 examples. Guide: [agents.md](./programming-with-gmail/agents.md)
- **working-with-amp-html-emails** — 17 examples. Guide: [agents.md](./working-with-amp-html-emails/agents.md)
- **working-with-exchange-ews-client** — 529 examples. Guide: [agents.md](./working-with-exchange-ews-client/agents.md)
- **working-with-exchange-webdav-client** — 41 examples. Guide: [agents.md](./working-with-exchange-webdav-client/agents.md)
- **working-with-ibm-notes** — 30 examples. Guide: [agents.md](./working-with-ibm-notes/agents.md)
- **working-with-imap-client** — 59 examples. Guide: [agents.md](./working-with-imap-client/agents.md)
- **working-with-microsoft-graph-client** — 32 examples. Guide: [agents.md](./working-with-microsoft-graph-client/agents.md)
- **working-with-mime-messages** — 121 examples. Guide: [agents.md](./working-with-mime-messages/agents.md)
- **working-with-outlook-items** — 409 examples. Guide: [agents.md](./working-with-outlook-items/agents.md)
- **working-with-outlook-storage-files** — 61 examples. Guide: [agents.md](./working-with-outlook-storage-files/agents.md)
- **working-with-pop3-client** — 40 examples. Guide: [agents.md](./working-with-pop3-client/agents.md)
- **working-with-smtp-client** — 15 examples. Guide: [agents.md](./working-with-smtp-client/agents.md)
- **zimbra** — 9 examples. Guide: [agents.md](./zimbra/agents.md)

## Boundaries
### ✅ Always
- Use explicit types for Aspose objects when clarity is needed.
- Dispose `SmtpClient`, `ImapClient`, `Pop3Client`, streams, and messages with `using` where applicable.

### ⚠️ Ask First
- Adding new NuGet dependencies besides `Aspose.Email`.
- Changing shared templates or pipeline settings.

### 🚫 Never
- Create multi-file projects; keep each example in one `.cs` file.
- Hardcode credentials, secrets, or license keys.
- Omit required namespaces—prefer fully qualified names if ambiguity arises.

## Testing Guide
- **Build**: `dotnet build --configuration Release --verbosity minimal`
- **Run**: `dotnet run`
Success = exit code 0 and no `CS####` compiler errors.

## Domain Knowledge (Email-specific)
- Use `MailMessage`/`MapiMessage` appropriately for MIME vs MAPI scenarios.
- Prefer `SecurityOptions.Auto` for SMTP/IMAP/POP TLS negotiation.
- Save MSG with `SaveOptions.DefaultMsgUnicode` to preserve UTF-8 content.

<!-- AUTOGENERATED:START -->
| Date | Run ID | Branch/Commit |
|------|--------|---------------|
| 2026-04-14 | `20260413_235146` | [examples/batch-20260413_235146](https://github.com/aspose-email/agentic-net-examples/tree/examples/batch-20260413_235146) |
<!-- AUTOGENERATED:END -->